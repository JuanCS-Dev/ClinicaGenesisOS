...
describe('patientService', () => {
  ...
  describe('age calculation', () => {
    ...
    it('should calculate age correctly when birthday has not occurred this year', async () => {
      ...
      const birthYear =
        birthMonth < today.getMonth() ? today.getFullYear() - 29 : today.getFullYear() - 30
    ...
    it('should calculate age correctly when in same month but day has not occurred', async () => {
      ...
      const thirtyYearsAgoSameMonth = new Date(today.getFullYear() - 30, today.getMonth(), birthDay)
  ...
  describe('subscribeToOne', () => {
    it('should subscribe to a single patient', () => {
      ...
      expect(onSnapshot).toHaveBeenCalled()
      expect(mockCallback).toHaveBeenCalledWith(expect.objectContaining({ name: 'Maria Silva' }))
      expect(typeof unsubscribe).toBe('function')
    })
  ...
  describe('addTag error handling', () => {
    it('should throw error when patient not found', async () => {
      ...
      await expect(patientService.addTag(mockClinicId, 'nonexistent', 'NewTag')).rejects.toThrow(
        'Patient not found: nonexistent'
      )
    })
  })
  ...
  describe('removeTag error handling', () => {
    it('should throw error when patient not found', async () => {
      ...
      await expect(patientService.removeTag(mockClinicId, 'nonexistent', 'Tag')).rejects.toThrow(
        'Patient not found: nonexistent'
      )
    })
  })
