...
const Application = lazy(() =>
  import('./pages/Application').then(m => ({ default: m.Application }))
)

// Landing subpages
const Manifesto = lazy(() =>
  import('./pages/landing/Manifesto').then(m => ({ default: m.Manifesto }))
)
const Tecnologia = lazy(() =>
  import('./pages/landing/Tecnologia').then(m => ({ default: m.Tecnologia }))
)
const Membership = lazy(() =>
  import('./pages/landing/Membership').then(m => ({ default: m.Membership }))
)
...
const WhatsAppMetrics = lazy(() =>
  import('./pages/WhatsAppMetrics').then(m => ({ default: m.WhatsAppMetrics }))
)
const NewPatient = lazy(() => import('./pages/NewPatient').then(m => ({ default: m.NewPatient })))
const EditPatient = lazy(() =>
  import('./pages/EditPatient').then(m => ({ default: m.EditPatient }))
)
      ...
      <div className="min-h-screen bg-[var(--color-genesis-soft)] flex font-sans text-[var(--color-genesis-text)] selection:bg-[var(--color-genesis-primary)]/20 selection:text-[var(--color-genesis-primary)]">
        ...
        <div className="flex-1 ml-64 flex flex-col min-w-0 transition-all duration-300">
          ...
          <main
            id="main-content"
            tabIndex={-1}
            className="flex-1 p-8 overflow-y-auto custom-scrollbar focus:outline-none"
          >
            <Outlet />
          </main>
        </div>
