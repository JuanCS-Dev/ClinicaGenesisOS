/**
 * Demo Patient Portal Layout
 *
 * Layout for the patient portal demo mode.
 * Adds demo banner and uses demo routes.
 *
 * @module components/patient-portal/DemoPatientPortalLayout
 */
...
import { useDemoPatientAuth } from '@/contexts/DemoPatientContext'
import { DemoBanner } from './DemoBanner'

// ============================================================================
// Navigation Items (demo paths)
// ============================================================================

const DEMO_NAV_ITEMS = [
  { path: '/portal/demo', icon: Home, label: 'Início', exact: true },
  { path: '/portal/demo/consultas', icon: Calendar, label: 'Consultas' },
  { path: '/portal/demo/historico', icon: FileText, label: 'Histórico' },
  { path: '/portal/demo/exames', icon: FlaskConical, label: 'Exames' },
  { path: '/portal/demo/receitas', icon: Pill, label: 'Receitas' },
  { path: '/portal/demo/mensagens', icon: MessageCircle, label: 'Mensagens' },
  { path: '/portal/demo/financeiro', icon: CreditCard, label: 'Financeiro' },
  { path: '/portal/demo/teleconsulta', icon: Video, label: 'Teleconsulta' },
]
...
function MobileNav({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  const { profile, logout } = useDemoPatientAuth()
  ...
  return (
    <aside className="hidden lg:flex flex-col w-64 bg-genesis-surface border-r border-genesis-border">
      <div className="p-6 border-b border-genesis-border">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-genesis-primary flex items-center justify-center">
...
export function DemoPatientPortalLayout(): React.ReactElement {
  const [mobileNavOpen, setMobileNavOpen] = useState(false)
  const { profile } = useDemoPatientAuth()

  return (
    <div className="min-h-screen bg-genesis-soft flex flex-col">
      {/* Demo Banner */}
      <DemoBanner />
          ...
          <header className="lg:hidden sticky top-0 z-40 bg-genesis-surface border-b border-genesis-border px-4 py-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <div className="w-8 h-8 rounded-lg bg-genesis-primary flex items-center justify-center">
...
export default DemoPatientPortalLayout
