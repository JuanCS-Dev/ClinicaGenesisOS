...
import { Shield, Eye, EyeOff, Send, RefreshCw, CheckCircle2, XCircle } from 'lucide-react'
    ...
    <div className="bg-genesis-surface rounded-2xl border border-genesis-border-subtle p-6">
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-genesis-muted">Configuradas via Firebase Secrets (seguro)</p>
        </div>
      </div>

      <div className="space-y-4">
        {/* Access Token Field */}
        <TokenField showToken={showToken} onToggleVisibility={() => setShowToken(!showToken)} />
...
function TokenField({
  ...
  return (
    <div>
      <label htmlFor="whatsapp-token" className="block text-sm font-medium text-genesis-text mb-1">
        Access Token
      </label>
      ...
      <p className="text-xs text-genesis-muted mt-1">Gerenciado via: WHATSAPP_TOKEN</p>
    </div>
  )
}
 ...
 */
function PhoneIdField({ connected }: { connected: boolean }): React.ReactElement {
  return (
    <div>
      <label
