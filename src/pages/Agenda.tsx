...
export const Agenda: React.FC = () => {
  ...
  // Filter for "Today" (ignoring time for comparison)
  const todayStr = new Date().toISOString().split('T')[0];
  const todaysAppointments = appointments.filter(app => app.date.startsWith(todayStr));
    ...
    <div className="h-[calc(100vh-8rem)] flex flex-col bg-white/60 backdrop-blur-md rounded-3xl border border-white shadow-soft overflow-hidden animate-enter">
      ...
      <div className="p-4 border-b border-white/50 flex justify-between items-center bg-white/80 backdrop-blur-md z-20 sticky top-0 shadow-sm">
        <div className="flex items-center gap-6">
           <div className="flex bg-gray-100/80 p-1 rounded-xl shadow-inner">
              <button className="px-5 py-1.5 text-xs font-bold bg-white shadow-sm rounded-[0.6rem] text-genesis-dark transition-all scale-100">Dia</button>
              <button className="px-5 py-1.5 text-xs font-bold text-genesis-medium hover:text-genesis-dark transition-colors">Semana</button>
              <button className="px-5 py-1.5 text-xs font-bold text-genesis-medium hover:text-genesis-dark transition-colors">Mês</button>
           </div>
           
           <div className="flex items-center gap-4">
             <button className="p-1.5 hover:bg-white rounded-lg transition-colors border border-transparent hover:border-gray-200 shadow-sm hover:shadow-md"><ChevronLeft className="w-5 h-5 text-genesis-medium" /></button>
             ...
             <button className="p-1.5 hover:bg-white rounded-lg transition-colors border border-transparent hover:border-gray-200 shadow-sm hover:shadow-md"><ChevronRight className="w-5 h-5 text-genesis-medium" /></button>
           </div>
        </div>
      ...
      <div className="flex-1 overflow-y-auto relative custom-scrollbar bg-genesis-soft/50">
         {hours.map(hour => (
           <div key={hour} className="flex min-h-[120px] group relative border-b border-gray-100/50">
             ...
             <div className="flex-1 relative mx-2">
               ...
               {todaysAppointments.filter(app => new Date(app.date).getHours() === hour).map(app => (
                 <div 
                  key={app.id}
                  className={`
                 ...
                 >
                    <div className="flex justify-between items-start mb-2">
                      <span className="font-bold text-sm text-genesis-dark tracking-tight">{app.patientName}</span>
                      <span className="text-[10px] font-bold text-genesis-dark/60 bg-white/80 px-2 py-1 rounded-lg backdrop-blur-sm shadow-sm">{app.durationMin} min</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <div className={`w-1.5 h-1.5 rounded-full ${app.procedure.includes('Nutri') ? 'bg-green-500' : 'bg-blue-500'}`}></div>
                        <p className="text-xs font-medium text-genesis-medium group-hover/card:text-genesis-dark transition-colors">{app.procedure}</p>
                    </div>
                 </div>
               ))}
             </div>
         ...
         <div 
            ...
            style={{ top: `${(new Date().getHours() - 8) * 120 + (new Date().getMinutes() / 60) * 120}px` }}
         >
            <div className="w-20 bg-red-50/80 text-red-500 text-[10px] font-bold text-right pr-2 backdrop-blur-sm">
               {format(new Date(), 'HH:mm')}
            </div>
            <div className="w-2.5 h-2.5 bg-red-500 rounded-full -ml-1.5 border-2 border-[#F5F5F7]"></div>
         </div>
      </div>
    </div>
  );
