 ...
 */
function extractCertificateInfo(pfxBase64: string, password: string): CertificateInfo {
...
export const storeCertificate = functions.https.onCall(
  async (
    ...
    // Validate auth, role (admin+ for certificate management), and clinic access
    const authContext = requireAuthRoleAndClinicV1(context, clinicId, ['owner', 'admin'])

    // Rate limiting for certificate operations
    await checkRateLimitForUser(authContext.userId, 'CERTIFICATE')
