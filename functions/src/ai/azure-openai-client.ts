 ...
 * - AZURE_OPENAI_KEY (via Firebase Secret Manager)
...
export function getAzureOpenAIConfig(): AzureOpenAIConfig {
  if (!azureConfig) {
    azureConfig = {
      endpoint:
        process.env.AZURE_OPENAI_ENDPOINT || DEFAULT_CONFIG.endpoint,
      apiKey: process.env.AZURE_OPENAI_KEY || '',
      deployment:
        process.env.AZURE_OPENAI_DEPLOYMENT || DEFAULT_CONFIG.deployment,
      apiVersion:
        process.env.AZURE_OPENAI_API_VERSION || DEFAULT_CONFIG.apiVersion,
    };
  }
  return azureConfig;
}
