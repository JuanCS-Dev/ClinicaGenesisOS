...
const KpiCard = ({ title, value, sub, icon: Icon, colorClass, iconBg }: any) => (
  <div className="group bg-white p-6 rounded-2xl border border-white shadow-soft hover:shadow-float hover:-translate-y-1 transition-all duration-300 ease-out">
    <div className="flex justify-between items-start mb-5">
      <div className={`p-3 rounded-xl ${iconBg} text-opacity-100 transition-transform duration-300 group-hover:scale-110`}>
...
export const Dashboard: React.FC = () => {
  const { patients, appointments, currentUser } = useStore();
  ...
  // Fake finance logic just for demo
  const estimatedRevenue = appointments.filter(a => a.status === Status.FINISHED).length * 350;

  return (
    <div className="space-y-8 animate-enter">
      <div className="flex justify-between items-end">
        <div>
          <h1 className="text-2xl font-bold text-genesis-dark tracking-tight mb-1">Olá, {currentUser.name}</h1>
          <p className="text-genesis-medium text-sm font-medium">Resumo operacional de hoje, {format(new Date(), "d 'de' MMMM", { locale: ptBR })}.</p>
        </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-2xl border border-white shadow-soft overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-auto max-h-[400px]">
            {appointments.length === 0 ? (
               <div className="p-8 text-center text-genesis-medium text-sm">Nenhuma consulta agendada.</div>
            ) : (
              appointments.sort((a,b) => new Date(a.date).getTime() - new Date(b.date).getTime()).map((app) => (
                <div key={app.id} className={`p-4 hover:bg-gray-50 transition-colors flex items-center justify-between group cursor-pointer border-b border-gray-50/50 last:border-0`}>
                  <div className="flex items-center gap-4">
                    <div className="flex flex-col items-center justify-center w-12 h-12 bg-genesis-soft rounded-xl text-genesis-dark border border-gray-100 group-hover:border-genesis-blue/30 group-hover:bg-white transition-all shadow-sm">
                      <span className="text-[10px] font-bold text-genesis-medium uppercase">{format(new Date(app.date), 'MMM', {locale: ptBR})}</span>
                      <span className="text-lg font-bold text-genesis-dark leading-none">{format(new Date(app.date), 'dd')}</span>
                    </div>
                    <div>
                      <h4 className="font-semibold text-genesis-dark text-sm mb-0.5">{app.patientName}</h4>
                      <div className="flex items-center gap-2 text-xs text-genesis-medium">
                        <Clock className="w-3 h-3" />
                        <span className="font-medium">{format(new Date(app.date), 'HH:mm')}</span>
                  ...
                  <div className="flex items-center gap-4">
                      <span className={`px-2.5 py-1 rounded-full text-[11px] font-semibold border 
                        ${app.status === Status.CONFIRMED ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 
                          app.status === Status.PENDING ? 'bg-amber-50 text-amber-600 border-amber-100' :
                          app.status === Status.IN_PROGRESS ? 'bg-purple-50 text-purple-600 border-purple-100' :
        ...
        <div className="bg-white rounded-2xl border border-white shadow-soft p-6 flex flex-col h-full">
          ...
          <div className="space-y-4 flex-1">
             <div className="flex items-start gap-3 p-4 bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md hover:border-red-100 transition-all cursor-pointer group">
                ...
                <div>
                  <p className="text-sm font-medium text-genesis-dark group-hover:text-red-600 transition-colors">Finalizar Prontuário</p>
                  <p className="text-[11px] text-genesis-medium mt-1">João Santos • Ontem às 16:00</p>
                </div>
             </div>
             {/* Static tasks for demo visual balance */}
             <div className="flex items-start gap-3 p-4 bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md hover:border-gray-300 transition-all cursor-pointer group">
                ...
                <div>
                  <p className="text-sm font-medium text-genesis-dark group-hover:text-black transition-colors">Confirmar agenda</p>
